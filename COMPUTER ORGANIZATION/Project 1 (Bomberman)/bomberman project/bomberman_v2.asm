...
main:
    ...
    # Print prompt4
    li $v0, 4              # Load immediate value 4 into $v0 for print_string syscall
    la $a0, prompt4        # Load address of prompt3 string into $a0
    syscall                # Execute syscall

    # Read grid
    lw $t0, row            # Load word from memory location of row into $t0,    t0 = row
    lw $t1, column         # Load word from memory location of column into $t1, t1 = column
    mul $t2, $t0, $t1      # Multiply row and column to get total number of grid elements
    la $t3, grid1          # Load address of grid into $t3
    
    li $t4, 0 # initialize counter, it is for printing newline after end of each column.
    
read_loop:             	    # Start of read loop

    beq $t2, $zero, end_read_loop  # If total number of elements is zero, end loop
    li $v0, 12             # Load immediate value 12 into $v0 for read_character syscall
    syscall                # Execute syscall
    sb $v0, 0($t3)         # Store the read character (in $v0) into current grid element
    addiu $t3, $t3, 1      # Increment address of current grid element
    addiu $t2, $t2, -1     # Decrement total number of elements
    
    # Increment counter
    addiu $t4, $t4, 1
    
    # If counter equals column size, print newline and reset counter
    beq $t4, $t1, print_newline1
    j read_loop            # Jump to start of loop
...
print_newline1:
    ...
    j read_loop            # Jump back to start of loop
    
end_read_loop:             # End of read loop
